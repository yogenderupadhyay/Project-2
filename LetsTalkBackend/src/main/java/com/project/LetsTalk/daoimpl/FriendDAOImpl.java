...
//********************************************list of friends*************************************************************
	...
	public List<User> friendList(User user) {
		/*return sessionFactory.getCurrentSession().createQuery("from Friend where (userName='"+userName+"'or friendUserName='"+userName+"') AND (status='Accepted')").list();*/
		Session session=sessionFactory.openSession();
		System.out.println("inside friend dao impl");
		SQLQuery query=session.createSQLQuery("select * from c_user where email_ID in (select userName from c_friend where friendUserName =? and status='Accepted' union select friendUserName from c_friend where userName=? and status='Accepted') and email_ID!=?");
...
//*****************************************List of pending friend request*****************************************************
	...
	public List<Friend> pendingFriendRequestList(String userName) {
		return sessionFactory.getCurrentSession().createQuery("from Friend where (friendUserName='"+userName+"' AND status='Pending')").list();
	}
...
//***************************************List of suggested friend request**************************************************
	...
	public List<User> suggestedPeopleList(User user) {
			Session session=sessionFactory.openSession();
			System.out.println("inside friend dao impl");
			SQLQuery query=session.createSQLQuery("select * from c_user where email_ID not in (select userName from c_friend where friendUserName =? union select friendUserName from c_friend where userName=?) and email_ID!=?");
			query.setString(0,user.getEmailID());
			query.setString(1,user.getEmailID());
			query.setString(2,user.getEmailID());
			query.addEntity(User.class);
